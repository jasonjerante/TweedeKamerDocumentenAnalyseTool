...
def slugify(text: str, max_len: int = 60) -> str:
    """Veilige slug voor bestandsnamen."""
    if not text:
        return "document"
...
def _safe_date_str(value: Any) -> str:
    """Zet date/datetime/str om naar YYYY-MM-DD of fallback."""
    if value is None:
        return "unknown-date"
...
def guess_extension(doc: Dict[str, Any]) -> str:
    """Bepaal extensie via file_ext of URL suffix."""
    ext = (doc.get("file_ext") or "").lower().strip(".")
    if ext:
        return ext

    url = (doc.get("download_url") or doc.get("source_url") or doc.get("url") or "").lower()
...
def make_unique_filename(doc: Dict[str, Any], title_max_len: int = 60) -> str:
    """
    Pattern:
      {dossier_id}_{date}_{doc_id}_{slug(title)}.{ext}
    """
